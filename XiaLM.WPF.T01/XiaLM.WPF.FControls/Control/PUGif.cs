...
namespace XiaLM.WPF.FControls.Control
{
    ...
    /// </summary>
    public class PUGif : Image
    {
        public static readonly DependencyProperty GIFSourceProperty = DependencyProperty.Register(
            "GIFSource", typeof(string), typeof(PUGif), new PropertyMetadata(OnSourcePropertyChanged));
        ...
        {
            PUGif gif = sender as PUGif;
            if (gif == null) return;
            if (!gif.IsLoaded) return;
            BindSource(gif);
        }
        private static void BindSource(PUGif gif)
        {
            gif.StopAnimate();
