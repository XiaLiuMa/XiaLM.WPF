...
using static XiaLM.WPF.FControls.Model.StyleEnum;
using System.Collections.ObjectModel;
using XiaLM.WPF.FControls.Model;
using System.Collections.Specialized;

namespace XiaLM.WPF.FControls.Control
{
    public class PUTreeView : TreeView
    {
        ...
        {
            if ((includeParent || !item.HasItems) && item.Header != null && item.Header.Equals(header))
                return item;
            if (item.HasItems)
            {
        ...
        {
            if ((includeParent || !item.HasItems) && item.Value != null && item.Value.Equals(value))
                return item;
            if (item.HasItems)
            {
                foreach (var tvi in item.Items)
